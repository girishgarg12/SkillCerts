  ...
  return (
    <>
      <CardContainer className="inter-var h-full w-full">
        <CardBody className="bg-[#1e293b]/40 relative group/card dark:hover:shadow-2xl dark:hover:shadow-blue-500/[0.1] dark:bg-black dark:border-white/[0.2] border-black/[0.1] w-full h-full min-h-[480px] rounded-xl p-4 border border-white/10 transition-all duration-300 backdrop-blur-sm flex flex-col justify-between overflow-hidden">
          <div className="absolute inset-0 h-full w-full bg-gradient-to-r from-blue-500/20 to-slate-500/20 transform scale-[0.80] rounded-full blur-3xl opacity-0 group-hover/card:opacity-30 pointer-events-none" />
          <Meteors number={10} />
          <div>
            ...
            <div className="mt-4">
              <CardItem
                translateZ="60"
                className="text-xl font-bold text-[#f8fafc] line-clamp-2 min-h-[56px]"
              >
                <Link to={`/courses/${course._id || course.id}`} className="hover:text-blue-400 transition-colors">
                  {course.title}
                </Link>
              </CardItem>
              ...
              {course.instructor && (
                <CardItem
                  as="p"
                  translateZ="40"
                  className="text-slate-400 text-sm max-w-sm mt-2 flex items-center gap-2"
                >
            ...
            <div className="flex items-center gap-2">
              {onWishlistToggle && (
                <CardItem
                  ...
                  className={`transition-all p-2.5 rounded-full hover:bg-white/10 border border-white/5 ${isInWishlist ? 'text-red-500 bg-red-500/10 border-red-500/20' : 'text-gray-400 hover:text-red-500'}`}
                >
              ...
              <CardItem translateZ={20}>
                <Button
                  ...
                  className="bg-blue-600 text-white hover:bg-blue-700 font-semibold shadow-lg shadow-blue-500/10"
                >
