...
const reorderLectureSchema = z.object({
  lectureIds: z.array(z.string()).min(1),
});

export const getSectionLectures = async (req, res) => {
  try {
    const lectures = await lectureService.getSectionLectures(
...
const convertTypes = (data) => {
  ...
  if (typeof data.isPreview === 'string')
    data.isPreview = data.isPreview === 'true';
...
export const createLecture = async (req, res) => {
  try {
    logToFile(
      `LECTURE_POST: User=${req.user?._id}, Section=${req.params.sectionId}, Body=${JSON.stringify(req.body)}, Files=${Object.keys(req.files || {})}`
    );
...
export const updateLecture = async (req, res) => {
  try {
    ...
    if (error instanceof z.ZodError) {
      console.error(
        '!!! LECTURE UPDATE ZOD VALIDATION FAILED !!!',
        error.issues
      );
      const firstError = error.issues[0];
      const detailMsg = firstError
...
export const reorderLectures = async (req, res) => {
  try {
    ...
    const updatedLectures = await lectureService.reorderLectures(
      req.params.sectionId,
      lectureIds,
      req.user
    );
