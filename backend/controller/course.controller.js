...
const convertTypes = (courseData) => {
  ...
  // totalDuration
  if (
    courseData.totalDuration !== undefined &&
    courseData.totalDuration !== null
  ) {
...
export const createCourse = async (req, res) => {
  try {
    ...
    console.log(
      'Course Data Prepared for Validation:',
...
export const getCourse = async (req, res) => {
  try {
    ...
    if (error.message === 'Course not published')
...
export const updateCourse = async (req, res) => {
  try {
    ...
    console.log(
      'Update Data Prepared for Validation:',
      JSON.stringify(courseData, null, 2)
    );
...
export const togglePublish = async (req, res) => {
  try {
    ...
    return ApiResponse.success(
      `Course ${result.published ? 'published' : 'unpublished'} successfully`,
      result
    ).send(res);
  } catch (error) {
    if (error.message === 'Course not found')
