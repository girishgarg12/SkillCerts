...
export const getCourseSections = async (req, res) => {
  try {
    ...
    console.log(
      `[DEBUG] GET_SECTIONS Request - Course: ${req.params.courseId}, User: ${req.user?._id}, Auth: ${authHeader ? 'Present' : 'Missing'}`
    );

    const sections = await sectionService.getCourseSections(
      req.params.courseId,
      req.user
    );
    return ApiResponse.success('Sections fetched successfully', sections).send(
      res
    );
  } catch (error) {
    logToFile(
      `!!! GET_SECTIONS ERROR !!!: ${error.message} - User: ${req.user?._id}`
    );
    ...
    if (error.message.startsWith('Course not published yet')) {
      if (!req.user) {
        return ApiResponse.unauthorized(
          'Authentication required to view draft curriculum'
        ).send(res);
      }
...
export const getSection = async (req, res) => {
  try {
    ...
    return ApiResponse.success('Section fetched successfully', section).send(
      res
    );
  } catch (error) {
...
export const deleteSection = async (req, res) => {
  try {
    ...
    return ApiResponse.success(
      'Section and its lectures deleted successfully'
    ).send(res);
  } catch (error) {
    if (error.message === 'Section not found')
      return ApiResponse.notFound('Section not found').send(res);
