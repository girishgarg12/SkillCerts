...
export const sectionService = {
  async getCourseSections(courseId, currentUser = null) {
    ...
    if (!course.published) {
      ...
      if (!isInstructor && !isAdmin) {
        ...
        throw new Error(
          `Course not published yet. Instructor: ${instructorId}, Current: ${currentUserId}`
        );
      }
    }
    ...
    return await Promise.all(
      sections.map(async (section) => {
        const lectures = await Lecture.find({ section: section._id }).sort({
          order: 1,
        });
        return {
          ...section.toObject(),
  ...
  async getSection(sectionId, currentUser = null) {
    ...
    if (!section.course.published) {
      const isInstructor =
        currentUser &&
        section.course.instructor.toString() === currentUser._id.toString();
      const isAdmin = currentUser && currentUser.role === 'admin';
