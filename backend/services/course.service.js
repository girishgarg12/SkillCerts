...
export const courseService = {
  ...
  async getAllCourses(filters) {
    ...
    if (search) {
      ...
      if (instructorIds.length > 0)
        query.$or.push({ instructor: { $in: instructorIds } });
    }
    ...
    return {
      ...
      debug: { query, count: courses.length },
    };
  },
  ...
  async getCourse(identifier, currentUser = null) {
    ...
    if (!course.published) {
      const isInstructor =
        currentUser &&
        course.instructor._id.toString() === currentUser._id.toString();
  ...
  async updateCourse(courseId, data, currentUser) {
    ...
    if (data.title) {
      ...
      if (await Course.findOne({ slug: newSlug, _id: { $ne: courseId } }))
        throw new Error('Course with this title already exists');
      data.slug = newSlug;
    }
