...
const run = async () => {
  ...
  try {
    ...
    for (const instructor of instructors) {
      const courses = await Course.find({ instructor: instructor._id }).sort({
        createdAt: -1,
      });
      ...
      for (const course of courses) {
        ...
        if (processedTitles.has(baseTitle)) {
          // Duplicate found!
          console.log(
            `Instructor ${instructor.name} has duplicate for "${baseTitle}": "${course.title}". Reassigning to Admin.`
          );
