...
const run = async () => {
  ...
  try {
    ...
    if (!systemUser) {
      console.log(
        'Admin not found. Creating default admin for orphan courses...'
      );
    ...
    if (!girish || !parna) {
      console.error(
        'Critical: Instructors not found! Please ensure Girish and Parna exist.'
      );
    ...
    for (const course of otherCourses) {
      ...
      await course.save();
      console.log(
        `Assigned "${course.title}" to System Admin (Hidden from Instructor List).`
      );
    }
